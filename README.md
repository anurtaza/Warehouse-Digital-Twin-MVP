...
- Интерактивная 3D-сцена склада с подсказками по ячейкам и AGV.
- Визуализация маршрутов AGV и текущих целевых точек.
- Имитация WMS-интеграции с очередью заказов, синхронизацией и обзором SKU.
- Оптимизация размещения SKU по приоритету и близости к зоне отгрузки.
- Немедленное обновление состояния при смене сценариев.
- Автоматическая начальная загрузка метрик и журнала событий.
